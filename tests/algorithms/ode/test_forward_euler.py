...
Unit tests for the functions in the andz.algorithms.ode.forward_euler module.
"""

import unittest

from andz.algorithms.ode.forward_euler import *
...
class TestForwardEuler(unittest.TestCase):
    ...
    def test_forward_euler_parameters_when_are_None(self):
        self.assertRaises(
            ValueError, forward_euler, None, self.b, self.n, 1, TestForwardEuler.f
        )
        self.assertRaises(
            ValueError, forward_euler, self.a, None, self.n, 1, TestForwardEuler.f
        )
        self.assertRaises(
            ValueError, forward_euler, self.a, self.b, None, 1, TestForwardEuler.f
        )
        self.assertRaises(
            ValueError, forward_euler, self.a, self.b, self.n, None, TestForwardEuler.f
        )

    def test_forward_euler_approx_parameters_when_are_None(self):
        self.assertRaises(
            ValueError,
            forward_euler_approx,
            None,
            self.b,
            self.n,
            1,
    ...
    def test_forward_euler_b_less_than_a(self):
        ...
        self.b = TestForwardEuler.gen_b(self.a, self.n, self.h)
        self.assertRaises(
            ValueError, forward_euler, self.a, self.b, self.n, 1, TestForwardEuler.f
        )
    ...
    def test_forward_euler_approx_f_is_not_callable(self):
        self.assertRaises(
            TypeError, forward_euler_approx, self.a, self.b, self.n, 1, None
        )
