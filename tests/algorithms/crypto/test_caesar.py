...
Unit tests for the functions in the andz.algorithms.crypto.caesar module.
"""

import unittest
from random import randint

from andz.algorithms.crypto.caesar import (
    MAX_MAPPED_INT,
    decrypt,
...
class TestCaesarCipher(unittest.TestCase):
    ...
    def template_test_multi_keys(self, n, size, total_keys):
        for _ in range(n):
            m = generate_random_string(size)
            keys = gen_rand_keys(
                total_keys, 1, MAX_MAPPED_INT - find_max_char_ord_value(m)
            )
