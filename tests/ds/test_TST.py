...
Unit tests for the classes and functions in the andz.ds.TST module.
"""
...
from andz.ds.TST import TST, _TSTNode
...
class TestTST(unittest.TestCase):
    ...
    def test_keys_with_prefix_empty_prefix(self):
        ...
        self.assertEqual(
            len(kwp), len(kwp_set)
        )  # I should not need to check this here!!!
    ...
    def test_keys_with_prefix_all_found(self):
        ...
        t.insert("occam", 2)
        self.assertEqual(
            sorted(t.keys_with_prefix("occa")), ["occam", "occasion", "occasionally"]
        )
...
class TestTSTNode(unittest.TestCase):
    ...
    def test_has_children_3(self):
        u = _TSTNode(
            "u", mid=_TSTNode("mid"), left=_TSTNode("left"), right=_TSTNode("right")
        )
        self.assertTrue(u.has_children())
