...
class HashTable:
    ...
    def __str__(self):
        return str([(k, v) for k, v in zip(self.keys, self.values) if k is not None])
...
def find_duplicates(ls):
    return [item for item, count in collections.Counter(ls).items() if (count > 1 and item is not None)]
