...
def lsd(a: list,
        ...
        key: callable = lambda string, index: ord(string[index])) -> list:
    ...
    if not all(len(x) == w for x in a):
        raise ValueError("the length of each string in a should be equal to "
                         "w = {}".format(w))
...
def radix_sort(a: list,
    ...
    if not (1 <= w <= int(math.log2(sys.maxsize)) + 1):
        raise ValueError("w should be an integer in the range "
                         "[1, {}]".format(int(math.log2(sys.maxsize)) + 1))

    for x in a:
        if not (0 <= x < 2 ** w):
            raise ValueError("element {} of 'a' cannot be represented with {} "
                             "bits".format(x, w))
