    ...
    for i in range(2, n + 2):
        ...
        for w in range(2, total_weight + 2):
            ...
            if weights[i - 2] > w - 1:
                ...
                profits[i][w] = max(
                    profits[i - 1][w],
                    values[i - 2] + profits[i - 1][w - weights[i - 2]],
                )
...
def _recursive_01_knapsack_aux(capacity: int, w: list, v: list, value: int) -> int:
    ...
    if len(w) > 0 and len(v) > 0:
        if w[-1] > capacity:  # We cannot include the nth item.
            ...
            value = max(
                v[-1]
                + _recursive_01_knapsack_aux(capacity - w[-1], w[:-1], v[:-1], value),
...
def _memoized_01_knapsack_aux(
    capacity: int, w: list, v: list, value: int, m: list
) -> int:
    """Either takes the last element or it doesn't.
    ...
    if len(w) > 0 and len(v) > 0:
        ...
        if w[-1] > capacity:  # We cannot include the nth item.
            ...
            value = max(
                v[-1]
                + _memoized_01_knapsack_aux(capacity - w[-1], w[:-1], v[:-1], value, m),
