...
def _recursive_lcs_length_aux(s1: str, n: int, s2: str, m: int, result: int) -> int:
    ...
    if n == 0 or m == 0:
        ...
        result = max(
            _recursive_lcs_length_aux(s1, n - 1, s2, m, result),
...
def _memoized_recursive_lcs_length_aux(
    s1: str, n: int, s2: str, m: int, result: list, matrix: list
) -> int:
    """Helper function of recursive_lcs_length."""
    if n == 0 or m == 0:
        return 0
        ...
        result = 1 + _memoized_recursive_lcs_length_aux(
            s1, n - 1, s2, m - 1, result, matrix
        )
    else:
        result = max(
            _memoized_recursive_lcs_length_aux(s1, n - 1, s2, m, result, matrix),
...
def bottom_up_lcs_length_partial(
    s1: str, s2: str, c1: str, c2: str, partial_weight: int = 0.5, matrix: bool = False
):
    ...
    for i in range(1, len(s1) + 1):
        ...
        for j in range(1, len(s2) + 1):
            ...
            # Partial match.
                ...
                m[i][j] = max(
                    m[i - 1][j], m[i][j - 1], m[i - 1][j - 1] + partial_weight
                )
...
if __name__ == "__main__":
    ...
    for a, b in examples:
        ...
        print(
            "bottom_up_lcs_length_partial(%s, %s, 'a', 'e') = %.1f" % (a, b, partial),
            end="\n\n",
        )
