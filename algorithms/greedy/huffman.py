...
def print_frequencies(frequencies):
    ...
    import operator
    print(tab.tabulate(sorted(frequencies.items(), key=operator.itemgetter(1)), headers=("Letter", "Frequency"), tablefmt="grid"))


def huffman(message, verbose=False):
...
def build_huffman_codes(root):
    """Starting from the _initialise node obtained by the huffman algorithm above,
...
def huffman_fibonacci_encoder(fn):
    """fn is supposed to be a list of tuples,
...
if __name__ == "__main__":
    root1 = huffman("MISSISSIPPI RIVER")
    root2 = huffman("Nelson")
